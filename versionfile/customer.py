...
class cusClass:
    def __init__(self, root):
        ...
        self.txt_bnkadd.place(x=700, y=220, height=110, width=350)

        lbl_paymenterm= Label(self.root, text="Payment Terms", font=("Century Gothic", 12, "bold"), bg="white").place(x=550, y=340)
        ...
        cmb_pterm.place(x=700, y=340, width=180)
        cmb_pterm.current(0)

        # Button function
        btn_save = Button(self.root, text="Save",command=self.add, font=("Century Gothic", 12), bg="darkgreen", fg="white", cursor="hand2").place(x=550, y=500, width=90, height=30)
        btn_udpate = Button(self.root, text="Update",command=self.update, font=("Century Gothic", 12), bg="darkblue", fg="white", cursor="hand2").place(x=650, y=500, width=90, height=30)
        btn_delete = Button(self.root, text="Delete",command=self, font=("Century Gothic", 12), bg="darkred", fg="white", cursor="hand2").place(x= 750, y=500, width=90, height=30)
        btn_clear = Button(self.root, text="Clear",command=self, font=("Century Gothic", 12), bg="darkgray", fg="white", cursor="hand2").place(x=850, y=500, width=90, height=30)
        btn_export = Button(self.root, text="Export",command=self, font=("Century Gothic", 12), bg="#2596be", fg="white", cursor="hand2").place(x=950, y=500, width=90, height=30)
        ...
        self.fetch_cus.bind("<ButtonRelease-1>", self.get_data)
        # self.show()
...
## Add data in data base
    ...
    def get_data(self, ev): # show data function
        data = self.fetch_cus.focus()
        content = (self.fetch_cus.item(data))
        row = content['values']
        # print(row)
    ...
    #         else:
    #             cur.execute("Select * from supplier where sid = ?", (self.var_cus_id.get(),))
    #             row = cur.fetchone()
    #             if row == None:
    ...
    #                 if op == True:
    #                     cur.execute("delete from supplier where sid=?",(self.var_cus_id.get(),))
    #                     con.commit()
    ...
    def fetch_cus(self):
        ...
        cur = con.cursor()
        try:
            cur.execute("Select cus_id, cus_name from customer")
            supp = cur.fetchall()
            self.cus_list.append("Empty")
            if len(supp)>0:
                del self.cus_list[:]
                self.cus_list.append("")
                for i in supp:
